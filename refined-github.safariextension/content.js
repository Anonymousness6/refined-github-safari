...
var elementReady = interopDefault(index);
...
if (typeof exports !== 'undefined') {
  ...
  /*jshint -W040 */
  commonjsGlobal.gitHubInjection = gitHubInjection;
  /*jshint +W040 */
}
});

var gitHubInjection = interopDefault(index$1);
...
'use strict';
const semver = interopDefault(require$$0);
...
var toSemver = interopDefault(index$2);
...
var escape = index$3.escape;

var jquery_slim_min = createCommonjsModule(function (module) {
...
var $ = interopDefault(jquery_slim_min);

const issueRegex = /([a-zA-Z0-9-_.]+\/[a-zA-Z0-9-_.]+)?#[0-9]+/;
const linkifyIssueRef = (repoPath, issue, attrs) => {
	if (/\//.test(issue)) {
...
const isRepoRoot = () => isRepo() && /^(\/?$|\/tree\/)/.test(getRepoPath()) && exists('.repository-meta-content');
...
const isCommit = () => isSingleCommit() || isPRCommit() || (isPRFiles() && exists('.full-commit'));

const isCompare = () => isRepo() && /^\/compare/.test(getRepoPath());

const hasCode = () => isRepo() && exists('.blob-code-inner');

const hasDiff = () => isRepo() && (isSingleCommit() || isPRCommit() || isPRFiles() || isCompare() || (isPR() && exists('.diff-table')));
...
const hasCommentForm = () => exists('.js-previewable-comment-form');
...
var copyToClipboard = interopDefault(index$5);
...
'use strict';
const mimicFn = interopDefault(require$$0$1);
...
var debounce = interopDefault(index$6);
...
function linkifyIssuesInTitles() {
	...
	const titleText = escape(title.textContent);
